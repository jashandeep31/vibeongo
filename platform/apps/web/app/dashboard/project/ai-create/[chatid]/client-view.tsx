...
const ClientView = ({ chatid }: ClientViewProps) => {
  ...
  return (
    <div
      aria-label={chat?.name ? `Chat ${chat.name}` : "Project creation chat"}
      className="bg-background text-foreground flex h-full flex-col justify-between"
    >
              ...
              return (
                <div key={item.id} className="flex flex-col gap-8">
                  ...
                  <div className="max-w-3xl">
                    {answer ? (
                      <p className="text-foreground text-2xl leading-snug whitespace-pre-wrap">
                        {answer.answer}
                      </p>
                    ) : (
                      <div
                        aria-label="Generating answer"
                        className="text-primary mt-8 flex items-center"
                      >
