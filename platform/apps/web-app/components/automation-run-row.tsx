          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            {run.project_session ? (
              <Button asChild variant="ghost" size="sm">
                <Link href={`/projects/${automation.project_id}/sessions/${run.project_session.id}`}>
                  View session
                </Link>
              </Button>
            ) : null}
          ...
          {expanded ? (
            <div className="pt-3">
              {sessionQuery.isLoading ? (
                ...
                <p className="text-destructive text-sm">Tasks could not be loaded.</p>
              ) : tasks.length === 0 ? (
                <p className="text-muted-foreground text-sm">No tasks in this session.</p>
              ) : (
